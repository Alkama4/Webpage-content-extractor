...
async def _fetch_all_webpages(conn: Connection) -> List[dict]:
    query = """
        SELECT webpage_id, url, page_name, run_time, is_active
        FROM webpages
        ORDER BY webpage_id;
    """
...
async def _create_webpage(conn: Connection, data: WebpageCreate) -> int:
    query = """
        INSERT INTO webpages (url, page_name, run_time, is_active)
        VALUES (%s, %s, %s, %s, %s);
    """
    try:
        last_id = await execute_mysql_query(
            conn,
            query,
            params=(data.url, data.page_name, data.run_time, data.is_active),
